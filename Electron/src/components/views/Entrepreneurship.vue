...
export default {
	...
	beforeMount() {
		this.$store.dispatch('setCurrentRoute', '/entreprenuership');
	},
	mounted() {
	...
	data() {
		return {
			...
			constant: {
				methodName: 'searchEntreprenuership'
			},
			error: {
	...
	computed: {
		Items() {
			...
			return this.$store.getters.entreprenuershipItems;
		},
	},
			...
			makeRequest(this.constant.methodName, this.helper.page)
				.then(res => {
					if (res && res.data && res.data.items) {
						...
						this.$store.dispatch('setEntreprenuershipItems', items);
